...
> **상태(2026-09):** 유지보수 모드입니다. 지원 버전은 v0.1.4이며 버그 수정만 계획합니다.
> 0.2 어댑터 분리 작업은 중단했습니다. 검사하는 것과 검사하지 않는 것은
> [지원 범위](#지원-범위)를 보세요.
...
**프로젝트 상태:** 유지보수 모드(버그 수정만)입니다. v0.1.4는 Maven Central에 공개되어 있고
계속 지원합니다. [코어와 어댑터를 분리하는 0.2](https://github.com/ym0506/routecontract/pull/62)는 중단했으며
5.5.2 지원은 미출시입니다. 공개 배포본의 설치·실행 검증은 유지관리자가 진행했습니다. 독립적인 외부
