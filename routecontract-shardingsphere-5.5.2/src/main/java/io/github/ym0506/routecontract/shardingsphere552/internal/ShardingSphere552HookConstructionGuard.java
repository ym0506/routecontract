...
/**
 * Non-recursive construction guard that runs before the 5.5.2 hook touches the core bridge.
 *
 ...
 */
final class ShardingSphere552HookConstructionGuard {
    ...
    private static final String SUPPORTED_VERSION = "5.5.2";
    ...
    private static final String HOOK_PROVIDER_552 =
            "io.github.ym0506.routecontract.shardingsphere552.internal.RouteContract552SqlExecutionHook";
    private static final String RUNTIME_PROVIDER_552 =
    ...
    private ShardingSphere552HookConstructionGuard() {
    }

    /** Checks actual JAR identities before core can link the exact hook's ABI. */
    static void verifyRuntimeResources() {
        Class<?> runtimeProvider = ShardingSphere552RuntimeAdapter.class;
    ...
    static void verify() {
        verifyUnnamedModules(RouteContract552SqlExecutionHook.class);
        ...
        synchronized (ShardingSphere552HookConstructionGuard.class) {
            ...
            Class<?> hookType = RouteContract552SqlExecutionHook.class;
    ...
    private static VerificationContext verificationContext() {
        ...
        Class<?> runtimeProvider = loadWithoutInitialization(RUNTIME_PROVIDER_552, loader);
    ...
    private static DescriptorLayout verifyDescriptorLayout(final ClassLoader loader) {
        // Descriptor classification must precede version-specific anchor linkage. With both
        // adapters present, either service-order must report the same dual-adapter marker instead
        // of whichever wrong-runtime marker happens to be encountered first.
    ...
    static void verifyAnchorVersions(
        ...
        if (!SUPPORTED_VERSION.equals(executorVersion)) {
            throw new IllegalStateException(
                    "RC_UNSUPPORTED_SHARDINGSPHERE_RUNTIME: exact adapter 5.5.2 observed "
                            + renderedVersion(executorVersion));
        }
    }
    ...
    static void verifyProviderNames(
        ...
        List<String> routeContractHooks = hookProviders.stream()
                ...
                .toList();
        long hook552Count = count(routeContractHooks, HOOK_PROVIDER_552);
        long runtime552Count = count(runtimeProviders, RUNTIME_PROVIDER_552);
        if (hook552Count > 1
                || runtime552Count > 1
                || routeContractHooks.size() != hook552Count
                || runtimeProviders.size() != runtime552Count) {
            throw new IllegalStateException(
                    "RC_MULTIPLE_ROUTE_CONTRACT_ADAPTERS: unexpected RouteContract provider descriptors are visible");
        }
        if (hook552Count == 0 || runtime552Count == 0) {
            throw new IllegalStateException(
                    "RC_ADAPTER_NOT_FOUND: exact 5.5.2 hook and runtime-adapter descriptors must both be visible");
        }
    }
    ...
    private static void verifyAdapterLayout(
        ...
        Class<?> runtimeProviderType = loadWithoutInitialization(RUNTIME_PROVIDER_552, loader);
    ...
    private static Class<?> loadRuntimeAnchor(final String className, final ClassLoader loader) {
        try {
            ...
            throw new IllegalStateException(
                    "RC_UNSUPPORTED_SHARDINGSPHERE_RUNTIME: required 5.5.2 anchor could not be linked: "
                            + className,
                    exception);
        }
    }
