...
OFFICIAL_LOCK = REPOSITORY_ROOT / "security" / "cyclonedx-cli.lock.json"
...
FAKE_OFFICIAL_VALIDATOR = f'''#!/usr/bin/env python3
import json
import os
from pathlib import Path
import sys
...
if not input_root.is_absolute() or input_root.resolve() != input_root:
    raise SystemExit("fake official validator: input root is not canonical")
if not input_root.name.startswith("audited-inputs."):
    raise SystemExit("fake official validator: input root is not an audited copy root")
...
def mutate(path):
    ...
    with path.open("a", encoding="utf-8") as stream:
        stream.write("mutated-around-official-validation\\n")

mutation = os.environ.get("TEST_OFFICIAL_MUTATION", "none")
if mutation == "pre-audited":
    mutate(input_root / audited_names[0])

for audited_name, original in zip(audited_names, original_paths):
    path = input_root / audited_name
    if not path.is_file() or path.is_symlink():
        raise SystemExit(f"fake official validator: invalid input {{audited_name}}")
    if mutation != "pre-audited" or audited_name != audited_names[0]:
        if path.read_text(encoding="utf-8") != f"fresh:{{original}}\\n":
            raise SystemExit(f"fake official validator: stale input {{audited_name}}")

if mutation == "during-audited":
    mutate(input_root / audited_names[1])
elif mutation == "post-original":
    mutate(repository_root / original_paths[0])
elif mutation not in ("none", "pre-audited"):
    raise SystemExit("fake official validator: unknown mutation mode")

Path(os.environ["TEST_OFFICIAL_LOG"]).write_text(
...
if command == "preflight":
    ...
    for flag, original in originals.items():
        audited = option(flag)
        if not audited.is_file() or audited.is_symlink():
            raise SystemExit(f"fake checker: missing {{flag}}")
        audited_parents.add(audited.parent)
        if audited.read_bytes() != (Path.cwd() / original).read_bytes():
            raise SystemExit(f"fake checker: copy differs for {{flag}}")
    if len(audited_parents) != 1 or not next(iter(audited_parents)).name.startswith(
        "audited-inputs."
    ):
        raise SystemExit("fake checker: inputs are not private audited copies")
    with Path(os.environ["TEST_EVENT_LOG"]).open("a", encoding="utf-8") as stream:
        stream.write("inventory-from-audited-copies\\n")
...
class FinalSupplyChainRunnerTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        shutil.copy2(OFFICIAL_LOCK, self.repository / "security" / OFFICIAL_LOCK.name)
        (self.repository / "scripts" / "verify-supply-chain-policy.py").write_text(
            FAKE_CHECKER, encoding="utf-8"
        )
        (self.repository / "scripts" / "validate-official-cyclonedx.py").write_text(
            FAKE_OFFICIAL_VALIDATOR, encoding="utf-8"
        )
        ...
        self.official_log = self.root / "official-inputs.json"
        self.event_log = self.root / "pipeline-events.txt"
        ...
        environment.update(
            {
                ...
                "TEST_OFFICIAL_LOG": str(self.official_log),
                "TEST_EVENT_LOG": str(self.event_log),
            }
        )
    ...
    def test_official_failure_stops_before_inventory_scanner_and_evidence(self) -> None:
        result = self.run_runner({"TEST_OFFICIAL_FAIL": "1"})
        ...
        self.assertEqual(
            [],
            list(
                (self.repository / "build/reports/security/derived").glob(
                    "audited-inputs.*"
                )
            ),
        )
        ...
        self.assertEqual(["preflight"], [call[0] for call in checker_calls])
        self.assert_no_final_output()

    def test_rejects_mutation_before_during_or_after_official_copy_validation(self) -> None:
        for mutation in ("pre-audited", "during-audited", "post-original"):
            with self.subTest(mutation=mutation):
                result = self.run_runner({"TEST_OFFICIAL_MUTATION": mutation})

                self.assertNotEqual(0, result.returncode)
                self.assertTrue(
                    "changed after it was bound" in result.stderr
                    or "differs from its generated source" in result.stderr,
                    result.stderr,
                )
                self.assertEqual(
                    ["generation", "official-twelve"],
    ...
    def test_rejects_each_generated_input_mutation_during_scanner_use(self) -> None:
        for relative in GENERATED_INPUTS:
            with self.subTest(generated_input=relative):
                ...
                self.assertNotEqual(0, result.returncode)
                self.assertIn("differs from its generated source", result.stderr)
                self.assert_no_final_output()
    ...
    def test_rejects_derived_inventory_mutation_during_scanner_use(self) -> None:
        ...
        self.assertNotEqual(0, result.returncode)
        self.assertIn("derived scanner inventory changed", result.stderr)
        self.assert_no_final_output()
