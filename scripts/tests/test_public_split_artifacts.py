...
SPEC = importlib.util.spec_from_file_location('public_split_artifacts', Path(__file__).parents[1] / 'public_split_artifacts.py')
receipt = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(receipt)


def valid_receipt(version='0.2.0'):
    artifacts = []
...
class PublicReceiptTest(unittest.TestCase):
    ...
    def test_rejects_versions_outside_reviewed_line(self):
        for version in ('0.1.2', '0.3.0', '0.2.01', '0.2.0-rc1', '0.2.0-SNAPSHOT', '../0.2.0', True, None, '0.2.' + '1'*100):
