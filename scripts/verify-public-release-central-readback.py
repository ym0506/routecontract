#!/usr/bin/env python3
"""Compare anonymous Central downloads with a verified single-artifact 0.1 bundle.
...
def _body_header(headers, name: str, default=None):
    # Real HTTPMessage preserves duplicates; a plain mapping is used only by
    # synthetic transports. Never silently choose one conflicting body header.
    ...
    if len(values) > 1:
        raise ReadbackError('AMBIGUOUS_BODY_HEADERS')
...
def _verify_response(opener, path: str, expected: bytes, deadline: float) -> None:
    ...
    try:
        with opener.open(request, timeout=_remaining(deadline)) as response:
            ...
            if _body_header(response.headers, 'Content-Encoding', 'identity').lower() != 'identity':
                raise ReadbackError('CONTENT_ENCODING')
            length = _body_header(response.headers, 'Content-Length')
...
def readback(snapshot: VerifiedSnapshot, evidence: Path, *, opener=None) -> int:
    ...
    summary = {
        'formatVersion': 1, 'kind': 'routecontract-public-release-central-readback',
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(snapshot.bundle_bytes)) as archive:
            for record in snapshot.entries:
                ...
                _write_json(evidence, 'public-readback.json', summary)
        if summary['verifiedEntryCount'] != 30:
...
def main(argv=None) -> int:
    ...
    if result:
        ...
        print(f'ROUTECONTRACT_PUBLIC_RELEASE_CENTRAL_READBACK_VERIFIED version={snapshot.version} entries=30; public consumers still required')
    return result
