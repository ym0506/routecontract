"""Closed reviewed-byte receipt shared by the public single-artifact 0.1 release checks.
...
MODULES = ('routecontract-shardingsphere-5.5',)
...
def stable_version(value: object) -> str:
    if not isinstance(value, str) or len(value) > 32 or re.fullmatch(r'0\.1\.(0|[1-9][0-9]*)', value) is None:
        raise ReceiptError('Expected a stable 0.1.x release version from 0.1.3 onwards')
    if int(value.rsplit('.', 1)[1]) < 3:
        raise ReceiptError('Expected a stable 0.1.x release version from 0.1.3 onwards')
    return value
...
def validate_consumer_receipt(document: object) -> dict:
    ...
    if not isinstance(artifacts, list) or len(artifacts) != 3:
        raise ReceiptError('Consumer receipt must contain exactly three payloads')
    expected = {}
