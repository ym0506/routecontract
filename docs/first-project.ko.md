...
주문 조회 테스트는 통과하는데, 쿼리를 바꾼 뒤부터 데이터 소스를 한 곳 더 조회한다면
어떻게 알아낼 수 있을까요? 이 가이드에서는 같은 주문을 반환하는 두 쿼리를 실행하고,
실행 횟수와 대상을 검사해 그 차이를 확인합니다. DB 설정과 검토된 기준 파일은 예제에
포함돼 있으며, Maven과 Gradle 중 하나를 골라 실행할 수 있습니다.
...
2. **자신의 fork**에서 **Actions**를 엽니다. 활성화 안내가 나오면 Actions를 활성화한 뒤
   워크플로 목록에서 **First project**를 선택합니다.
...
4. 새 실행을 열고 완료 후 **Summary**에서 세 단계를 확인합니다.
   ...
   | 단계 | 검사하는 주문 | 실행 시도 / 데이터 소스 별칭 수 | 계약 결과 |
   | --- | --- | --- | --- |
   ...
   **예상한 거부와 정상 복구를 모두 확인해야 체험 워크플로가 성공합니다.** 범위 조회 테스트
...
워크플로는 기준 파일이 없는 첫 실행에서도 관측 결과를 기준으로 자동 승인하지 않는지 확인합니다.
비교할 때는 예제에 포함된 합성 데이터용 기준 파일을 사용하며, 이 파일을 바꾸지 않습니다. 자신의 애플리케이션 기준을
승인하는 과정은 별도입니다. 체험 후 [자신의 테스트 하나에 적용](#자신의-테스트와-ci로-옮기기)하세요.

**Run workflow**가 보이지 않으면 자신의 fork인지, 기본 브랜치에 워크플로가 있는지
...
| `RCM201` | 훅이 보고한 물리 JDBC 실행 시도가 상한을 넘었습니다. | 기준은 최대 1회, 관측은 2회. |
| `RCM202` | 사용한 데이터 소스의 서로 다른 별칭 수가 상한을 넘었습니다. | 기준은 최대 1개, 관측은 2개. |

**별칭(alias)**은 설정된 데이터 소스에 붙이는 민감하지 않은 이름입니다. 예제는 `ds_0`을
`orders-even`, `ds_1`을 `orders-odd`로 표시합니다. 실행 예산은 허용 상한입니다.
이 수치가 물리 테이블 개수나 성능 저하를 뜻하지는 않습니다.

실패 리포트를 읽은 뒤 조회 조건 옵션 없이 원래 명령을 다시 실행하면 `MATCH`로 돌아옵니다.
...
JUnit에서 가져옵니다. `orders`와 `expectedOrders`는 기존 테스트의 조회 객체와 기대 반환값입니다.
...
JSON 기준 파일·관측 결과 파일·리포트를 읽거나 쓰지 않으므로, 기존 리포트 파일은 이전 실행의 결과입니다.
First project 워크플로도 이 방식의 통과 → 실패 → 원복을 검사합니다.

위 두 검사는 실행 시도와 데이터 소스 개수만 확인합니다. SQL 해시값, 파라미터 타입 구조, 저장된 데이터 소스 집합을
...
기본 `check` 모드는 검토를 마친 JSON 기준 파일(`baseline`)과 이번 실행 결과(`candidate`)를
비교합니다. 자신의 테스트에서도 JSON 비교를 사용하려면 먼저 실행 결과를 검토해서 기준을
정해야 합니다. 아래 명령으로 예제의 기존 기준을 유지한 채 새 경로에서 이 과정을 연습할 수
있습니다.
...
`build/routecontract/candidate.json`을 열고 반환값 검사, 실행 시도 수, 데이터 소스 별칭,
SQL 해시값과 파라미터 타입 구조, 허용 상한을 테스트 코드·샤딩 설정과 함께 검토하세요.
예제에서는 실행 시도 1회와 관측 별칭 1개를 허용합니다. 실행 수집에 성공했다고 기준 승인이나
CI 비교까지 통과한 것은 아닙니다.
...
코드 리뷰에서 실행 결과를 검토하고 기준 파일을 테스트와 함께 커밋해야 합니다.
...
`MATCH`가 나와야 합니다. 기준 파일이 없으면 비교는 실패하며, 이번 실행 결과를 기준으로 자동
승인하지 않습니다.
...
표시할 수 있습니다. 테스트 실패를 그대로 유지하고, 각 실행의 새 결과를 Actions 아티팩트로 보관하세요.
기준 변경은 코드와 함께 검토합니다.
