...
| G5: independent use | Aug 22, 23:00 | At least one non-author attempts the exact quick start and creates or confirms a public feedback record; `v0.1.0-rc1` assets/checksums are accessible | Report "no external validation" if none occurs. Use a recorded clean VM as reproducibility evidence but never call it an external user |
...
| Aug 21 | Cut `v0.1.0-rc1`; verify archive, checksums and dependency contents | E09 candidate; invite targeted reviewers with a five-minute task and feedback template | Do not call an RC a stable release or fabricate download/adoption numbers |
