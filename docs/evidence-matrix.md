...
| E12 | License, notices, dependency inventory and SBOM | Project license, NOTICE where required, third-party licenses, dependency lock/inventory, generated SBOM, license/security scan and manual review notes | `verified-local` | Apache-2.0 `LICENSE`, `NOTICE`, `THIRD_PARTY.md`, pinned wrapper, locks, checksum metadata and CycloneDX generation are public. The candidate exact-revision gate checks aggregate/direct profile partition, JSON/XML pairs, generated POM/runtime closure, licenses and a pinned offline OSV database, and the release integration retains only a sanitized summary. The aligned graph fixes ShardingSphere-JDBC at 5.5.3, strictly constrains Calcite Core/linq4j to 1.42.0, excludes JTS I/O Common while retaining JTS Core, and reports zero findings with zero policy vulnerability exceptions for the pinned graph/database snapshot. This point-in-time result is not a vulnerability-free claim. Exactly one MySQL OCI manual license review remains unresolved. Final evidence still requires this exact graph on the submitted revision, a successful immutable final-tag run and the owner's MySQL OCI review. |
...
| 오픈소스SW 조합; 현재 한계; 라이선스 검토 상태 | E02, E08, E11, E12 | SPI/Testcontainers/thin-JAR/SBOM roles, exact scope, the retained JTS Core/excluded JTS I/O boundary and the unresolved MySQL OCI owner review without implying legal clearance |
