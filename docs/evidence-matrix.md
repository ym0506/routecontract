...
| E12 | License, notices, dependency inventory and SBOM | Project license, NOTICE where required, third-party licenses, dependency lock/inventory, generated SBOM, license/security scan and manual review notes | `verified-local` | Apache-2.0 `LICENSE`, `NOTICE`, `THIRD_PARTY.md`, pinned wrapper, dependency locks, SHA-256 verification metadata and CycloneDX generation are public. The main CI generated verified aggregate JSON/XML and published them as an artifact with a recorded Actions digest; [the exact revision and expiry are documented](public-ci-evidence.md). Component assertions cover the project, Connector/J's FOSS exception and the test-only MySQL container. An exact-revision local audit of the PR #19 dependency candidate observed 3 reviewed OSV matches, compared with 12 in the older main dependency graph. The candidate result uses exact, expiring exceptions in the ShardingSphere 5.5.3 test/example graph, but the runner and result are not yet public or release evidence and must not be cited as such. This row still needs the reviewed gate merged, regeneration from the final tag, a final-tag workflow artifact and final manual/legal review. |
...
- An exact-revision local audit of the PR #19 dependency candidate observed 3 reviewed OSV matches versus 12 in the older main graph. Separately, its 50-test root build and 1-test standalone consumer were green. The runner hardening and result are still isolated candidate work; no public revision, release workflow or submission artifact retains them.
...
|---|---|---|---|
| Code structure and completeness — 6 | E02, E03, E05, E06, E07, E08 | Small public API, isolated 5.5.3 adapter, fail-closed activation, complete lifecycle, deterministic model, real-MySQL coverage and clean build | Medium-low: public Linux/MySQL evidence exists; a tagged release and independent install still do not |
| Open-source growth — 6 | E09, E10, E13, E14 | Public release, usable contribution path, independent install feedback, visible issue/PR discussion and an upstream/community touchpoint | Critical: real Issue/PR history has begun, but release, external user and upstream contact remain pending |
...
| Community expansion | 10 | E09, E10, E13, E14 | Release, contribution process, public feedback and community interaction with honest outcomes | Critical: project history has begun, but no release, external user or upstream response exists yet |
| Open-source appropriateness | 10 | E02, E08, E09, E11, E12, E13 | Reusable package, public source/history, clear license, dependency transparency, extensions and non-project-specific API | Public source/history/license/SBOM exist; stable distribution and external consumption remain pending |
...
| License | 5 | E12 | SBOM and license scan match the shipped revision; notices and redistribution obligations are reviewed | Medium: a public development-revision SBOM and an experimental local scan/policy path exist, but final-release SBOM regeneration, manual review and the required submission form remain; no scan artifact is currently part of release evidence |
...
| SBOM | `artifact-ready` | The current public revision has a digest-identified aggregate JSON/XML Actions artifact; regenerate, verify, checksum and submit it from the final public release revision, then complete the required manual license review. The experimental local OSV audit is not part of the workflow artifact or submission claim. Neither inventory nor any local scan is a legal conclusion |
