...
The RouteContract v0.1 non-bundled distribution boundary is distinct from the
unresolved upstream metadata gap. The main JAR accepts only `.class` entry
...
revalidate them, prove the source archive matches the final tagged tracked
tree, and retain the owner's source/provenance review before closing the
release-specific distribution review. The upstream gap stays visible in the
aggregate test/example SBOM, and the review reopens if those payload invariants
change or a JTS/Mahout published dependency enters the release. This is a
...
It is a test/example-only transitive dependency reached through
...
The exception expires on 2026-08-27 and must then be removed, renewed with new
...
The fixture uses strict Gradle constraints for `aggdesigner-algorithm:6.1`,
`json-smart:2.4.10` and `accessors-smart:2.4.9`, matching the versions managed
by the ShardingSphere 5.5.3 parent POM. In the pinned scan those constraints
remove the prior legacy Commons Lang and `json-smart:2.5.0` findings without
changing the published RouteContract POM or runtime closure.
