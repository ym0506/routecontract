...
쿼리를 바꾼 뒤에도 원하는 주문은 그대로 조회될 수 있습니다. 하지만 그 주문을 찾으려고 DB를 하나 더 조회하거나, 실행 횟수는 같은데 다른 DB를 조회할 수 있습니다. 반환값 검사와 실행 검사는 이런 차이를 각각 확인합니다.
...
| --- | --- | --- |
| 기대한 주문을 반환한다 | 기존 반환값 검사 | 어느 DB를 조회했는지 |
...
[횟수는 같고 대상만 바뀌는 실험](application-evaluations.ko.md#횟수는-같지만-데이터-소스가-바뀌는-경우)도 있습니다. 실행 횟수만 세면 놓칠 수 있는 조건을 보여줍니다. 이 실험은 합성 데이터를 사용했으며, 라우팅 코드를 가져온 애플리케이션에 결함이 있다는 뜻은 아닙니다.
...
<a id="2-작업-스레드의-이벤트가-어느-요청에-속하는지-구분합니다"></a>

## 2. 작업 스레드의 이벤트가 어느 작업에 속하는지 구분합니다
...
같은 작업도 스레드 실행 순서에 따라 이벤트가 들어오는 순서는 달라질 수 있습니다. 이 순서나 시각, 무작위 수집 식별자를 기준 파일에 저장하면 의미 없는 차이가 생깁니다. 실행 명세는 구조가 같은 항목을 묶어 정렬하고, 같은 항목이 몇 번 있었는지 기록합니다. 실행 항목은 같고 순서만 바뀌면 같다고 판단하지만, 한 번 실행하던 것을 두 번 실행하면 차이로 남습니다.

원본 SQL과 바인딩 값 대신 SQL의 SHA-256 해시값과 파라미터의 개수·타입을 저장합니다. 데이터 소스 이름은 호출자가 검토한 별칭으로 바꿉니다. **보관하는 데이터를 줄이는 방식이며 익명화를 보장하지 않습니다.** 추측하기 쉬운 SQL은 해시도 대조할 수 있고, 타입 이름은 내부 구조를 드러낼 수 있습니다. 별칭 대응 관계를 잘못 바꾸면 대상 변경을 숨길 수도 있습니다. 해시값은 재작성된 SQL 원문으로 계산하므로 두 쿼리의 의미가 같은지 증명하지 않습니다.
...
검증기는 먼저 명세 형식의 버전과 작업 ID가 맞는지, 수집 결과가 계약 검사를 진행할 조건을 갖췄는지 확인합니다. 이어서 허용 상한과 구조 차이를 검사합니다. 리포트를 생성하는 것만으로 빌드가 실패하지는 않습니다. `assertMatched`와 CLI는 일치하지 않는 모든 결과를 거부합니다. 별도 API인 `assertPassesBlockingChecks`는 검토만 필요한 `REVIEW_REQUIRED` 결과를 허용하므로, 이 정책이 필요할 때 명시적으로 선택해야 합니다. 차이가 의도한 것인지 SQL과 설정을 확인한 뒤 기준 변경을 결정하세요.
...
- [0.1.4 공개 설치 검증](evidence/release-0.1.4-central.md): 유지관리자가 공개 배포 파일과 실제 Java·MySQL·빌드 도구 조합을 검사한 기록입니다.
